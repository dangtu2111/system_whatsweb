...
function phone_codes() {
	return [
		...
		'KP' => ['name' => 'Korea, Democratic People\'s Republic of', 'dial_code' => '+850'],
		'KR' => ['name' => 'Korea, Republic of', 'dial_code' => '+82'],
		'LA' => ['name' => 'Lao People\'s Democratic Republic', 'dial_code' => '+856'],
...
function user_redirect($type=false) {
...
function title($str) {
	return $str . ' &mdash; ' . config('app.name');
}
...
function is_dashboard($path, $yes) {
	if(request()->is(config('whatsweb.dashboard') . ($path ? '/' . $path : ''))) {
		return $yes;
	}else{
		return false;
	}
...
function statistic() {
	return new App\Facades\Statistic;
}

function stat_date($opr, $date) {
	return $opr . ' "' . $date . '"';
}

function user_type_html($type) {
	$type = strtoupper($type);
	if($type == 'MEMBER') {
		return '<div class="badge badge-info">Member</div>';
	}else{
...
function route_type($route, $arr = []) {
	$route = !auth()->check() ? $route : (is_backend() ? $route : 'dashboard.' . $route);

	return route($route, $arr);
}
...
function user_prefix() {
	return is_backend() ? 'backend' : 'dashboard';
}

function is_demo() {
	return (config('whatsweb.demo') && is_backend() ? true : false);
}